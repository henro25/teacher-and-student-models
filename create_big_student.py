...
class Config:
    ...
    temperature = 5.0
    alpha = 0.7
    teacher_model_path = "resnet18_cifar10_tailored_epoch200.pth"
...
# From here: https://www.kaggle.com/code/faressayah/cifar-10-images-classification-using-cnns-88
...
# def distill_teacher_to_router_with_clusters(teacher, router, loader, optimizer, device, big_class_map, epochs=5):
#     teacher.eval()
#     router.train()
...
#             # Total loss
#             # total_loss_batch = distill_loss + 0.01 * diversity_loss
...
#             predicted_clusters = router_probs.argmax(dim=1)
#             ground_truth_clusters = torch.tensor([class_to_cluster[label.item()] for label in targets], device=device)
#             correct_assignments += (predicted_clusters == ground_truth_clusters).sum().item()
#             total_samples += targets.size(0)
...
def main():
    ...
    teacher.load_state_dict(torch.load("resnet18_cifar10_tailored_epoch200.pth"))
    teacher.to(device)
